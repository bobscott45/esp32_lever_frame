...
### Added
- Implemented hardware PWM backlight fading (`display_hal_fade_backlight`) for smooth, premium soft-start and dimming transitions during sleep/wake cycles on hardware that supports it (Waveshare ESP32-P4).
## [1.6.1] - 2026-06-26

### Changed
- Reverted the experimental "Pseudo-Stage 3" Wi-Fi disconnection hacks from `main.c` as they are no longer necessary when the board's LDO capacitor flaw is physically corrected. Sleep mode now correctly leaves Wi-Fi connected for instant wake and uninterrupted OpenLCB event reception.
- Updated `README.md` to officially document the 100uF capacitor hardware fix for the Waveshare ESP32-S3 brownout issue.
...
### Added
- Added hardware display selection to `menuconfig` for easier switching between S3 and P4 board variants.

### Changed
- Updated documentation with clearer build instructions for multiple targets using separate build directories.
- Removed redundant S3 hardware glitch warnings from documentation.

## [1.4.0] - 2026-06-25

### Added
- Added support for the Waveshare ESP32-P4-WIFI6-Touch-LCD-4.3 display module.

### Changed
- Updated default configuration to a prototypical interface.
- Updated documentation to compare ESP32-S3 and ESP32-P4 hardware capabilities.

### Fixed
- Fixed touch alignment issues on the P4 display.
- Fixed Wi-Fi connection issues.
- Fixed LVGL UI locking glitch.
